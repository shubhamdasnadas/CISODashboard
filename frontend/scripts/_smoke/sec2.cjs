...
(async () => {
  await esbuild.build({
    entryPoints: [path.join(root, 'scripts/_smoke/RTTest.jsx')],
    bundle: true, format: 'cjs', platform: 'node',
    loader: { '.jsx': 'jsx', '.js': 'jsx' }, jsx: 'automatic',
    logLevel: 'silent', outfile: path.join(root,'scripts','_smoke','secBundle2.cjs'),
    external: ['@react-pdf/renderer','react','react/jsx-runtime','react-dom'],
  });
  const RT = require(path.join(root,'scripts','_smoke','secBundle2.cjs'));
