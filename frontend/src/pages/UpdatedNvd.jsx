...
// Dynamic window defaults: start = yesterday, end = today. These are recomputed
// on every render so "tomorrow" the start becomes the day before tomorrow, etc.
...
export default function UpdatedNvd() {
  ...
  const loadCreds = useCallback(async () => {
    ...
    try {
      const r = await api.get('/updated-nvd/credentials');
      const d = r.data || {};
      if (d.apiKey) {
        setHasCreds(true);
      ...
      // are dynamic (yesterday -> today) and the user picks the window explicitly;
      // loading the old stored date here was causing the sync to always use the
      // first-run date. We only surface the last-used window as read-only info.
      setWindow({
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      <div>
        <h1 className="text-xl font-bold text-[var(--foreground)]">Updated NVD — Modified CVEs Sync</h1>
        <p className="text-sm text-[var(--muted)] mt-0.5">
          Pick a date window (lastModStartDate → lastModEndDate) and sync only the CVEs modified in that range.
          Each CVE is upserted by cve_id (new ones inserted, existing ones updated).
        </p>
      </div>
      ...
      <div className="bg-[var(--card-bg)] border border-[var(--card-border)] rounded-2xl p-4 shadow-sm space-y-4">
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          >
            {syncing ? 'Updating CVEs…' : 'Updated CVE'}
          </button>
        </div>
        ...
        <p className="text-[11px] text-[var(--muted)] leading-relaxed">
          ...
          (e.g. <code>?lastModStartDate=2026-08-26T00:00:00.000Z&amp;lastModEndDate=2026-08-27T12:00:00.000Z</code>).
          View synced rows on the <a href="/nvd" className="text-indigo-600 dark:text-indigo-400 hover:underline">NVD</a> page.
        </p>
      </div>
    </div>
  );
